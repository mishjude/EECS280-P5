...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static Node *copy_nodes_impl(Node *node) {
    ...
    Node * return_pointer = new Node{}; //new node structure pointer
    return_pointer->datum = node->datum;
    return_pointer->left = copy_nodes_impl(node->left);
    return_pointer->right = copy_nodes_impl(node->right);
  ...
  static Node * insert_impl(Node *node, const T &item, Compare less) {
    ...
    //only check if head node is equal 
    //ASSERT(!less(node->datum, item) && !less(item, datum));
