...
private:
  ...
  class PairComp {
    public: 
      bool operator()(const std::pair<Key_type, Value_type> &pair1, 
      const std::pair<Key_type, Value_type> &pair2) const {
...
public:
  ...
  Iterator find(const Key_type& k) const{
    std::pair<Key_type, Value_type> helper;
    helper.first = k;
    helper.second = Value_type();
    return bst.find(helper);
  }
  ...
  Value_type& operator[](const Key_type& k) { //will do this later
    Iterator i = find(k);
    if (i != end()) {
        return i->second;
    } else {
        return insert({k, Value_type()}).first->second;
        //loosely based off hint link
    }
  }
  ...
  std::pair<Iterator, bool> insert(const Pair_type &val) {
    ...
    if (i != end()) {
      ...
      Iterator insert = bst.insert(val);
      return {insert, true};
    }
  }
